...
/** Simulation for [[I2cBitController]].
  *
  * The bit controller is the only block that touches the I²C bus directly, so
  * this sim is the foundation for all the higher-level tests. We don't bother
  * with a second [[I2cIoBus]] participant for most cases — a one-sided observer
  * that snoops the wires is enough, because the controller is the only driver.
  * The arbitration test is the exception: there we need a second driver to
  * actively pull SDA low while the controller has it released.
  *
  * Approach: instead of asserting an exact cycle-by-cycle waveform (which is
  * brittle and re-implements [[BusTiming]] in the test), we focus on
  * protocol-level invariants:
  *
  *   - SCL pulse counts match the number of bits issued.
  *   - Every Start/Stop edge satisfies its spec dwell ±1 cycle of entry
  *     latency.
  *   - WriteBit produces the expected SDA value during the SCL high phase.
  *   - ReadBit captures the SDA value driven by the sim during the SCL high
  *     phase.
  *   - Arbitration loss flips `arbLost` and releases both lines within one
  *     cycle.
  *   - Clock stretching extends the SCL high phase by exactly the number of
  *     cycles the sim holds SCL low.
  *
...
object I2cBitControllerSim {
  ...
  private def smokeStartStop(): Unit = {
    ...
    SimConfig.withWave
      .compile(I2cBitController(cfg))
      .doSim("smoke-start-stop") { dut =>
        ...
        assert(
          dut.io.bus.scl.write.toBoolean,
          "bus.scl should rest released after Stop"
        )
        assert(
          dut.io.bus.sda.write.toBoolean,
          "bus.sda should rest released after Stop"
        )
        assert(
          sdaEdges.falling >= 1,
          s"expected SDA fall during Start, got ${sdaEdges.falling}"
        )
        assert(
          sdaEdges.rising >= 1,
          s"expected SDA rise during Stop, got ${sdaEdges.rising}"
        )
        ...
        assert(
          sclEdges.falling == 1,
          s"expected 1 SCL fall, got ${sclEdges.falling}"
        )
        assert(
          sclEdges.rising == 1,
          s"expected 1 SCL rise, got ${sclEdges.rising}"
        )
  ...
  private def writeByte(): Unit = {
    val cfg = I2cConfig(clkFreqHz = 12000000, busSpeed = BusSpeed.Standard)
    SimConfig.withWave.compile(I2cBitController(cfg)).doSim("write-byte") {
      dut =>
        dut.clockDomain.forkStimulus(period = 10)
        dut.io.cmd.valid #= false
        dut.io.txBit #= false
        ...
        val pattern = Seq(true, false, true, false, false, true, false,
          true) // 0xA5 MSB-first
        var sclPulses = 0
        ...
        assert(
          sclPulses == pattern.size,
          s"expected ${pattern.size} SCL pulses, got $sclPulses"
        )
        assert(
          !dut.io.arbLost.toBoolean,
          "arbLost should not fire when bus is mirrored"
        )
  ...
  private def readByte(): Unit = {
    val cfg = I2cConfig(clkFreqHz = 12000000, busSpeed = BusSpeed.Standard)
    SimConfig.withWave.compile(I2cBitController(cfg)).doSim("read-byte") {
      dut =>
        dut.clockDomain.forkStimulus(period = 10)
        dut.io.cmd.valid #= false
        dut.io.txBit #= false
        ...
        val pattern = Seq(false, true, true, false, true, false, false,
          true) // 0x69 MSB-first
  ...
  private def repeatedStart(): Unit = {
    val cfg = I2cConfig(clkFreqHz = 12000000, busSpeed = BusSpeed.Standard)
    SimConfig.withWave.compile(I2cBitController(cfg)).doSim("rep-start") {
      dut =>
        dut.clockDomain.forkStimulus(period = 10)
        dut.io.cmd.valid #= false
        dut.io.txBit #= false
  ...
  private def arbitrationLoss(): Unit = {
    ...
    SimConfig.withWave.compile(I2cBitController(cfg)).doSim("arb-loss") { dut =>
      ...
      assert(
        dut.io.arbLost.toBoolean,
        "arbLost must be set after losing SDA contention"
      )
      assert(
        dut.io.bus.scl.write.toBoolean,
        "SCL must be released after arb loss"
      )
      assert(
        dut.io.bus.sda.write.toBoolean,
        "SDA must be released after arb loss"
      )
  ...
  private def clockStretching(): Unit = {
    ...
    SimConfig.withWave.compile(I2cBitController(cfg)).doSim("clock-stretch") {
      dut =>
        dut.clockDomain.forkStimulus(period = 10)
        dut.io.cmd.valid #= false
        dut.io.txBit #= false
        ...
        println(
          s"OK: clock stretching extends bit by $actualDelta sim-time (>= $expectedDelta)"
        )
    }
  }
