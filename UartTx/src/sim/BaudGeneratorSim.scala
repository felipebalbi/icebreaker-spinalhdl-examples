...
/** Standalone sim for [[BaudGenerator]].
  *
  * Strategy
  *   We deliberately pick a *small* clkFreqHz / baudRate ratio
  *   (1_000_000 / 100_000 = 10) so that bit periods are short enough to
  *   simulate many of them quickly, while still being non-trivial. With
  *   accWidth = 24, the rounded phaseInc gives an essentially exact
  *   10-cycle average period.
  *
  * What we check
  *   1. With `enable = false`, no ticks ever appear.
  *   2. After raising `enable`, the *first* tick lands within
  *      [ticksPerBit, ticksPerBit + 1] cycles (DDS jitter is ±1 cycle,
  *      plus 1 cycle for the registered tick output).
  *   3. Subsequent inter-tick spacings stay within {ticksPerBit-1,
  *      ticksPerBit, ticksPerBit+1} cycles.
  *   4. Each tick is exactly one cycle wide.
  *   5. The *average* spacing over many ticks matches `ticksPerBit`
  *      to within a small tolerance — proving DDS accuracy.
  *   6. Dropping `enable` cleanly stops ticks; re-enabling restarts
  *      the cadence from a known phase.
  *
...
object BaudGeneratorSim {
  def main(args: Array[String]): Unit = {
    ...
    val cfg = UartTxConfig(
      clkFreqHz = 1_000_000,
      baudRate  = 100_000   // => ticksPerBit = 10
    )
    ...
    SimConfig
      ...
      .doSim { dut =>
        ...
        assert(dut.io.tick.toBoolean,
          s"no tick within $maxFirst cycles after enable")
        assert(firstWait <= maxFirst,
        ...
        println(f"avg gap = $avg%.4f cycles (target $ticksPerBit, err = ${err*100}%.4f%%)")
