...
/** Transmit shift register: parallel-in, serial-out (LSB first).
  ...
  *     deliberate — never lose an incoming byte to a stray shift pulse.
  ...
  * were ever read before any `load` (it shouldn't be — the FSM gates
...
case class TxShiftReg(cfg: UartTxConfig) extends Component {
  val io = new Bundle {
    ...
    /** Current bit on the wire — combinationally tied to bit 0 of the
