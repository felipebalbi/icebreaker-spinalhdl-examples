...
/** UART transmitter (TX-only half).
  *
  * Top-level shape: a Stream of bytes in, a serial line out, plus a CTS
  * input for (optional, currently unused) hardware flow control.
  *
  ...
  *   parity : optional, one bit (when `cfg.parity != None`) — not yet wired
  ...
  * `data.ready` is held low while a frame is in flight, so the producer
  * (or an upstream FIFO) waits without losing bytes.
  *
  * @param cfg compile-time configuration (clock freq, baud, frame format)
  */
case class UartTx(cfg: UartTxConfig) extends Component {
  val io = new Bundle {
    /** Byte input, ready/valid handshake.
      ...
      * asserts `data.ready` only when it is idle (and `cts` permits).
      * A transfer occurs on any cycle where both are high; from that
      * cycle the byte is "owned" by UartTx and shifted out over the next
      * ~10 bit periods, during which `data.ready` stays low.
      */
    val data = slave Stream(Bits(cfg.dataBits bits))

    /** Serial output line. Idles high. Drops low for the start bit, then
      * carries `cfg.dataBits` data bits LSB-first, then `cfg.stopBits`
      * high stop bits, then returns to idle.
      */
    val tx = out Bool()
    ...
    /** Clear-To-Send input from the far end (active high = "I can receive").
      ...
      * end has buffer space again. Currently wired but unused; it will
      * be AND-ed with the FSM's idle signal once the FSM exists.
      */
    val cts = in Bool()
  }
...
/** Verilog generation entry point.
  *
  * For now this generates the bare `UartTx` core. Once a `UartTxTop`
  * wrapper exists (clock domain + pin mapping for the iCE40), swap
  * the `generateVerilog(...)` argument to that.
  *
