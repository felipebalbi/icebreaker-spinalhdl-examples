...
/** End-to-end smoke sim for `PwmFadeTop`.
  *
  * Uses a small `width` and a tiny `prescalerWidth` so a full breath fits in a
  * reasonable sim window. Open the resulting `.vcd`/wave file in GTKWave (look
  * under `simWorkspace/`) to see:
