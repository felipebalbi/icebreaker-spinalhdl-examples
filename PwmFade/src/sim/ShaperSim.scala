...
/** LUT-content tests for the shapers.
  *
  * For each `Shaper` subclass, sweeps `phase` over the entire input range and
  * compares `io.duty` to the same Scala formula used to build the LUT. Catches
  * off-by-one and rounding bugs in the table generation.
  *
  * Notes
  *   - `dut.io.phase #= i` is sim-DSL: drive a value onto a hardware signal in
  *     the testbench.
  *   - `sleep(1)` lets combinational logic settle before reading. Enough
  *     because the shapers use `readAsync`. With `readSync` we'd need
  *     `dut.clockDomain.waitSampling()` instead.
  *
...
object ShaperSim {
  ...
  private def checkIdentity(width: Int): Unit = {
    ...
    SimConfig.compile(IdentityShaper(width)).doSim { dut =>
      for (i <- 0 until depth) {
        ...
        assert(got == BigInt(i), s"Identity[$i] expected $i got $got")
      }
    }
  }
  ...
  private def checkSine(width: Int): Unit = {
    ...
    SimConfig.compile(SineShaper(width)).doSim { dut =>
      for (i <- 0 until depth) {
        ...
        sleep(1)
        val want =
          math.round(0.5 * (1.0 - math.cos(2.0 * math.Pi * i / depth)) * maxV)
  ...
  private def checkGamma(width: Int, gamma: Double): Unit = {
    ...
    SimConfig.compile(GammaShaper(width, gamma)).doSim { dut =>
      ...
      dut.io.phase #= maxV; sleep(1);
      assert(dut.io.duty.toBigInt == BigInt(maxV))
    }
  }
}
