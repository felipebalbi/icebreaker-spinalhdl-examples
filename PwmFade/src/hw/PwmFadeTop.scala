...
/** Top-level: clock domain + prescaler + modulator + PWM carrier.
  ...
  *
  *   - PWM carrier (fast): `PwmCore` counts every cycle and wraps at 2^width.
  *     At 12 MHz, width=12 => ~2.93 kHz PWM (invisible to the eye, perfect for
  *     analog brightness).
  *   - Duty modulation (slow): a free-running `prescaler` of `prescalerWidth`
  *     bits generates a 1-cycle `tick` pulse every 2^prescalerWidth cycles. The
  *     `Modulator` only updates its phase on those ticks, so the duty value
  ...
  * reset wire is generated.
  *
  * Swapping waveforms Change `kind` (and optionally `gamma`) in
