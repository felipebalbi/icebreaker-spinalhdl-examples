...
/** Counter-comparator PWM generator.
  ...
  * the declared width). Each cycle, `io.pwm` is high while `counter < io.duty`
  * and low otherwise — a textbook PWM.
  *
  * Notes
  *   - `Reg(...) init(0)` declares a register with a known reset value.
  *     Combined with `BOOT` reset in the top, the iCE40 cold starts the counter
  *     at 0.
  *   - The counter is unconditional (no enable). It MUST run at the full clock;
  *     the PWM frequency is derived from it.
  */
