...
/** Composite duty modulator: `PhaseGen` + selected `Shaper`.
  *
  * The only thing `PwmFadeTop` needs to know about. Hides the phase-vs-shape
  * split behind a single (tick in, duty out) port.
  *
  * Pattern
  ...
  * shaper kind (e.g. `Modulator_Sine`). Helpful when reading synthesis reports
  * — otherwise everything would be called `ModulatorImpl`.
  */
