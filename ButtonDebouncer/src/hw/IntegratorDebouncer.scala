...
/** Saturating up/down-counter debouncer.
  ...
  *   - Each cycle, increment the counter when the synchronised input is
  *     high; decrement when it is low. Saturate at `0` and `2^width - 1`.
  *   - Latch `state := True` once the counter saturates high and
  *     `state := False` once it saturates low.
  *
  * Why this works Bounces look like brief excursions to the wrong rail in
  * the middle of a long run of correct samples. Each wrong sample only
  * undoes one cycle of progress, so a few bounces in 2^width cycles cannot
  * flip the output.
  *
  * Choosing `width` Roughly: minimum debounce time at 12 MHz is
  *   `2^width / clkFreqHz`. For width=16 that is ~5.5 ms; width=18 gives
  *   ~22 ms. Tactile buttons settle in well under 10 ms, so width=16 is a
  *   reasonable default.
  *
  * Notes
  *   - The output `state` only ever changes on a *full* saturation, so the
  *     `rising`/`falling` pulses are guaranteed to fire at most once per
  *     real press / release.
  *   - `state` is registered, so `rising = state && !prev` cannot
  *     glitch. The dedicated `prev` register avoids the level-vs-edge
  *     CDC trap that biting `io.raw` directly would suffer from.
  */
